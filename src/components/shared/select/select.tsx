...
const Select = forwardRef<TextInput, SelectProps>(({ placeholder, value, onPress, onClear, className, isEditable = true, isDate, error }, ref) => {
  const formattedValue = value && isDate && dayjs(value).isValid() ? fromUTCToLocal(value) : value;

  return (
    <View className="w-full">
      <View className={`flex-row items-center border rounded-lg pl-1 py-2 ${error ? 'border-red-500' : 'border-gray-300'} ${className}`}>
