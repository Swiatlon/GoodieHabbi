      ...
      <TouchableOpacity onPress={onPress} activeOpacity={isScrolling ? 1 : 0.7}>
        <View className="flex-row flex-nowrap gap-2 mb-2">
          {tags.map((tag, index) => (
            ...
            >
              <Ionicons name="pricetag-outline" size={12} color={tag.textColor} className="mr-1" />
              <Text className="text-[10px] font-medium" numberOfLines={1} ellipsizeMode="tail" style={{ color: tag.textColor }}>
                {tag.value}
              </Text>
            </View>
          ))}
