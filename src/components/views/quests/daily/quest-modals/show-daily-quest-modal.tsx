...
import QuestSeasonExtended from '../../reusable/show-quest-modal/quest-seasons-extended';
...
interface ShowDailyQuestItemModalProps {
...
const ShowDailyQuestItemModal: React.FC<ShowDailyQuestItemModalProps> = ({ quest, isVisible, onClose, deleteQuest, onUpdate }) => {
  ...
  const handleDelete = () => {
    deleteQuest({ id: quest.id })
      .then(() => {
        showSnackbar({
          text: t('quests.daily.showModal.deletedSuccess'),
      ...
      .catch(() => {
        showSnackbar({
          text: t('quests.daily.showModal.deletedError'),
    ...
    <Modal
      ...
      footer={
        <View className="flex-row flex-wrap justify-between mt-4 w-full">
          <Button
            label={t('quests.daily.showModal.closeButton')}
            styleType="primary"
            onPress={onClose}
          ...
          />
          <Button
            label={t('quests.daily.showModal.editButton')}
            styleType="accent"
            onPress={onUpdate}
          ...
          />
          <Button
            label={t('quests.daily.showModal.deleteButton')}
            styleType="danger"
...
export default ShowDailyQuestItemModal;
