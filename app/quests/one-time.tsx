...
import { useGetAllQuestsQuery } from '@/redux/api/one-time-quests';

const OneTimeQuests: React.FC = () => {
  const [isConfigModalVisible, setIsConfigModalVisible] = useState(false);
  const [isAddQuestModalVisible, setIsAddQuestModalVisible] = useState(false);
  const { data: fetchedQuests = [], isLoading } = useGetAllQuestsQuery({});
  ...
  if (isLoading) {
    return <Loader message="Fetching quests..." size="large" />;
  }
  ...
  return (
    <View className="flex-1 p-4">
      ...
      <FlatList
        data={sortedData as IGetOneTimeQuestsResponse[]}
      ...
      <ConfigModal<IGetOneTimeQuestsResponse>
